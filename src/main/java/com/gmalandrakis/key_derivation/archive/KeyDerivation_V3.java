...
πρώτο με πρώτο, δεύτερο με δεύτερο, κ.ο.κ)
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(getKeyFromInput(str.getBytes())));
    ...
    public static byte[] getKeyFromInput(byte[] input) {
        var keyAsMultiArray = deduceKey(input, input.length);
     ...
     */
    public static byte[][] deduceKey(byte[] oldByteSequence, int originallength) {
        var byteSequence = new byte[32];
        var len = oldByteSequence.length;
        ...
        if (len > 32) {
            ...
            byteSequence = flatten(deduceKey(Arrays.copyOfRange(oldByteSequence, 0, 32), originallength));
            for (int i = 1; i < parts_of_32_bytes; i++) {
                    int l = 0;
                for (int j = i*32; j < i*32 + 32; j++) {
                    byte temp = (byte) (byteSequence[l] ^ ((byte)i));
                    oldByteSequence[j] ^= temp;
                    ++l;
                }
                byteSequence = flatten(deduceKey(Arrays.copyOfRange(oldByteSequence, 0, 32), i));
            }
               /* var a = toSignedLong(Arrays.copyOfRange(byteSequence, 0, 8)) + 1*i;
                ...
                for (int j = i*32; j < i*32 + 32; j++) {
                    byte temp = (byte) (byteSequence[j] ^ ((byte)i));
                    oldByteSequence[j] ^= temp;
                }
    ...
    public static byte[] arrayFlattening(byte[][] x) {
